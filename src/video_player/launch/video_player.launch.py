...
def generate_launch_description():
    # Config File
    config_file = PathJoinSubstitution([FindPackageShare("video_player"), "config", LaunchConfiguration("config_file")])

    # Set use_sim_time parameter
    use_sim_time = SetParameter(name="use_sim_time", value=LaunchConfiguration("use_sim_time"))

    # Spawn Node
    launch_video_player_node = Node(
        package="video_player",
        executable="main.py",
        name="video_player_node",
        output="screen",
        parameters=[config_file],
        arguments=["--ros-args", "--log-level", ["video_player_node:=", LaunchConfiguration("log_level")]],
    )

    ld = LaunchDescription(ARGUMENTS)
    ld.add_action(use_sim_time)
    ld.add_action(launch_video_player_node)
