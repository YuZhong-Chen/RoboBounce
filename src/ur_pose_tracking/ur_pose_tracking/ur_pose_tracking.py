...
class URPoseTracking(Node):
    def __init__(self):
        ...
        self.declare_parameter("pid_rot_kp", 1.0)
        self.declare_parameter("pid_rot_ki", 0.0)
        self.declare_parameter("pid_rot_kd", 0.0)
    ...
    def publish_servo_command_callback(self):
        ...
        current_euler = tf_transformations.euler_from_quaternion(current_q)
        ...
        target_euler = tf_transformations.euler_from_quaternion(target_q)
        ...
        err_roll = self.normalize_angle(target_euler[0] - current_euler[0])
        err_pitch = self.normalize_angle(target_euler[1] - current_euler[1])
        err_yaw = self.normalize_angle(target_euler[2] - current_euler[2])
