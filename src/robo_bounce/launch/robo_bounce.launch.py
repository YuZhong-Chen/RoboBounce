...
def generate_launch_description():
    # Config File
    config_file = PathJoinSubstitution([FindPackageShare("robo_bounce"), "config", LaunchConfiguration("config_file")])

    # Set use_sim_time parameter
    use_sim_time = SetParameter(name="use_sim_time", value=LaunchConfiguration("use_sim_time"))

    # Spawn Node
    launch_robo_bounce_node = Node(
        package="robo_bounce",
        executable="main.py",
        name="robo_bounce_node",
        output="screen",
        parameters=[config_file],
        arguments=["--ros-args", "--log-level", ["robo_bounce_node:=", LaunchConfiguration("log_level")]],
    )

    ld = LaunchDescription(ARGUMENTS)
    ld.add_action(use_sim_time)
    ld.add_action(launch_robo_bounce_node)
